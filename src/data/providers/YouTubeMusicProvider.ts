...
// ΓöÇΓöÇΓöÇ Cache Helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Normalization Functions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function normalizeYouTubeThumbnails(thumbnails?: { url: string; width: number; height: number }[]): JioSaavnImage[] {
...
function getBestThumbnailUrl(thumbnails?: { url: string; width: number; height: number }[]): string {
  const best = getBestThumbnail(thumbnails);
  return best ? upscaleYouTubeThumbnail(best.url) : "";
}

function getBestThumbnail(thumbnails?: { url: string; width: number; height: number }[]): { url: string; width: number; height: number } | null {
...
function extractVideoId(track: any): string {
  ...
  for (const candidate of candidates) {
    const value = readString(candidate).replace(/^youtube_/, "");
...
function normalizeArtists(raw: unknown): { name: string; id?: string }[] {
...
function normalizeThumbnails(raw: unknown): { url: string; width: number; height: number }[] {
...
function stripVisualMetadata(value: unknown): string {
  const cleaned = cleanSongTitle(readString(value));
  return normalizeComparableText(cleaned)
    .split(" ")
...
function isOfficialMusicVideoType(value: unknown): boolean {
  ...
  return type === "music_video_type_omv" || type.includes("official_music_video") || type.endsWith("_omv") || type === "video";
}
...
function scoreOfficialVisualCandidate(seed: Song, candidate: Song, index: number): number | null {
  ...
  // Official video type is the strongest signal
  if (isOfficialType) score += 100;
  
  // Title match quality
  if (titleScore.strong) score += 50;

  // Simple keyword checks
  if (candidateText.includes("official")) score += 40;
  if (candidateText.includes("music video")) score += 25;

  // Duration match - most reliable indicator
  ...
  if (seedDuration > 0 && candidateDuration > 0) {
    ...
    if (diff <= 5) score += 60;      // Nearly exact match
    else if (diff <= 15) score += 35;
    else if (diff <= 30) score += 15;
    else if (diff > 120) return null;  // Too different, skip
  }
...
function compactSongCandidates(candidates: (Song | null | undefined)[]): Song[] {
...
// ΓöÇΓöÇΓöÇ Timeout Wrapper ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function getEndpointCandidates(
  ...
  if (isPrivateDevelopmentApiUrl(appBase)) {
    ...
    if (appBase === productionBase) {
      // Using production URL directly ΓÇö no need to duplicate
...
// ΓöÇΓöÇΓöÇ API Functions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async function searchPipedVideosDirectly(query: string, limit = 8): Promise<Song[]> {
  const instances = [
  ...
  const promises = instances.map(async (baseUrl) => {
    try {
  ...
  try {
    ...
    return items.flatMap((item: any) => {
      const videoId = item.url ? item.url.split("v=")[1] || item.url.split("/").pop() || "" : "";
      if (!videoId || videoId.length !== 11) return [];
      const authorName = item.uploaderName || "";
      return [{
        id: `youtube_${videoId}`,
        videoId,
        title: item.title || "",
        ...
        thumbnails: [{ url: item.thumbnail || "", width: 300, height: 300 }],
        source: "youtube",
    ...
    logger.warn("[PipedSearchDirect] All Piped instances failed:", error);
    return [];
  }
}
...
async function prefetchYouTubeMusicAutoplay(
  ...
  streams: { videoId: string; stream: YouTubeMusicAudioStream }[];
} | null> {
  const cleanVideoId = extractVideoId({ videoId: readString(videoId).replace(/^youtube_/, "") });
  if (!cleanVideoId) return null;
...
export async function getYouTubeMusicVisualVideoId(song: Song): Promise<string | null> {
  ...
  // Helper: search using simple, accurate queries
  const searchForOfficialVideo = async (queries: string[]): Promise<string | null> => {
    for (const query of queries) {
      try {
        ...
        const results = await searchYouTubeMusicVideos(query, 8);
  ...
  if (!audioVideoId) {
    ...
    if (song.title && song.artist) {
      ...
      const queries = [
        `${cleanTitle} ${song.artist} official video`,
        `${song.title} ${song.artist} music video`,
      ];
  ...
  // Step 3: Targeted search with simple, accurate queries
  if (song.title && song.artist) {
    const cleanTitle = cleanSongTitle(song.title);
    const queries = [
      `${cleanTitle} ${song.artist} official video`,
  ...
  // Step 4: Use audio video ID as final fallback
...
function normalizeHomeShelfPlaylistCard(item: any): YouTubeMusicPlaylistCard | null {
  ...
  const author = readString(item?.author || item?.owner || item?.channel?.name) || description.split("ΓÇó")[0]?.trim();
...
function getMoodCategoryItems(json: any): { title: string; params: string }[] {
